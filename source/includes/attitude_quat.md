...
ROS-Service Name: /<namespace>/mavros/imu/data
ROS-Service Type: sensor_msgs/Imu, below is its description
...
```shell
rosservice call /<namespace>/mavros/imu/data "twist:
  header:
    seq: 0
...
ros::NodeHandle nh;
ros::ServiceClient client = nh.serviceClient<core_api::PositionSet>("mavros/imu/data");
...
def setpoint_local_position(lx, ly, lz, yaw, tolerance= 0.0, async = False, relative= False, yaw_rate_valid= False, body_frame= False):
    rospy.wait_for_service('namespace/mavros/imu/data')
    try:
        handle = rospy.ServiceProxy('namespace/mavros/imu/data', PositionSet)
