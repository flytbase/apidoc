...
```shell
rosservice call /<namespace>/setup/actuator_testing "twist:
  header:
    seq: 0
...
```cpp--ros
#include <core_api/ActuatorTesting.h>

ros::NodeHandle nh;
ros::ServiceClient client = nh.serviceClient<core_api::ActuatorTesting>("setup/actuator_testing");
core_api::ActuatorTesting srv;
...
def setpoint_local_position(lx, ly, lz, yaw, tolerance= 0.0, async = False, relative= False, yaw_rate_valid= False, body_frame= False):
    rospy.wait_for_service('namespace/setup/actuator_testing')
    try:
        handle = rospy.ServiceProxy('namespace/setup/actuator_testing', ActuatorTesting)
